          ...
          <table className="table-premium">
            <thead>
              <tr>
                ...
                <th className="text-center">{salaryCategories.find(c => c.id === 'meal_allowance')?.name || 'Meal Allowance'}</th>
                {activeCustomCategories.map(category => (
                  <th key={category.id} className="text-center">{category.name}</th>
                ))}
                <th className="text-center">Total</th>
                <th className="text-center">Salary History</th>
              ...
              {activeStaff.map((member, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-4 text-sm font-semibold text-green-600 text-center">₹{calculateMemberTotalSalary(member).toLocaleString()}</td>
                    <td className="px-3 py-4 text-center">
                      <button
                  ...
                  {editingCategory === cat.id ? (
                    ...
                    <>
                      <div className="flex items-center gap-2">
                        ...
                        {cat.isBuiltIn && <span className="text-xs text-blue-400 italic px-1.5 py-0.5 bg-blue-500/20 rounded">Built-in</span>}
